...
"""
    ...
    def obtain_token(self):
        ...
        try:
            ...
            raise RuntimeError("Timeout while retrieving oauth token. Please run gcert.") from e
        except subprocess.CalledProcessError as e:
            logging.error("Error while trying to retrieve token: %s", e)
            raise RuntimeError(f"Failed to obtain token via {token_proc}: {e}") from e
    ...
    def list_artifacts(self, bid, build_target):
        ...
        if "artifacts" in response:
            return [a["name"] for a in response["artifacts"]]
        else:
            raise RuntimeError(
