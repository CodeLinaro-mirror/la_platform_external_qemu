...
def fetch_and_merge(args):
    ...
    qemu = Path(args.aosp) / "third_party" / "qemu"
...
def main():
    ...
    parser.add_argument(
        "--dest",
        type=str,
        default=find_aosp_root() / "third_party" / "qemu",
        help="Destination path for the final BUILD.bazel file (usually $AOSP/third_party/qemu)",
    )
    parser.add_argument(
        "--aosp",
        type=str,
