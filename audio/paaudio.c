        ...
        int *rerror)
{
    ...
    /* Streams are initialized asynchronously, wait here until they are PA_STREAM_READY. */
    while (true) {
        ...
        case PA_STREAM_CREATING:
            ...
            if (g_get_monotonic_time() > deadline_us) {
                AUD_log(AUDIO_CAP, "Timeout waiting for stream to become ready");
                goto fail;
            }
            break;

        default:
            AUD_log(AUDIO_CAP, "Stream failed while waiting to become ready");
            goto fail;
        }
    }
