...
static void qpa_volume_out(HWVoiceOut *hw, Volume *vol)
{
    ...
    op = pa_context_set_sink_input_volume(c->context, stream_index, &v,
                                          NULL, NULL);
    if (!op) {
        qpa_logerr(pa_context_errno(c->context),
    ...
    op = pa_context_set_sink_input_mute(c->context, stream_index, vol->mute,
                                        NULL, NULL);
    if (!op) {
        qpa_logerr(pa_context_errno(c->context),
...
static void qpa_volume_in(HWVoiceIn *hw, Volume *vol)
{
    ...
    op = pa_context_set_source_output_volume(c->context, stream_index, &v,
                                             NULL, NULL);
    if (!op) {
        qpa_logerr(pa_context_errno(c->context),
    ...
    op = pa_context_set_source_output_mute(c->context, stream_index, vol->mute,
                                           NULL, NULL);
    if (!op) {
        qpa_logerr(pa_context_errno(c->context),
