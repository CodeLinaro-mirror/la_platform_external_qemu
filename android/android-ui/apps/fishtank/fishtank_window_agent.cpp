...
static const QAndroidEmulatorWindowAgent sFishtankQAndroidEmulatorWindowAgent = {
        ...
        .fold = [](bool is_fold) -> bool {
            if (emulator_window_recorder_state_get().state ==
                RECORDER_RECORDING) {
                return false;
            }
        ...
        .setUIDisplayRegion =
                [](int x, int y, int w, int h, bool ignoreOrientation) {
                    // The UI display region needs to be updated when we
                    // add additional displays, or when we resize the
                    // screen. In the embedded case we do not have a
                    // visible UI, so we do not need to update display
                    // regions.
                    if (const auto win = EmulatorQtWindow::getInstance()) {
        ...
        .getMultiDisplay = [](uint32_t id,
                              ...
                              uint32_t* flag,
                              bool* enabled) -> bool {
            LOG(WARNING) << "TODO(jansene): Hookup gRPC";
            // if (const auto ins =
            // android::MultiDisplay::getInstance()) {
            //     return ins->getMultiDisplay(id, x, y, w, h, dpi,
            //     flag,
            //                                 enabled);
            // }
            return false;
...
const QAndroidEmulatorWindowAgent* const gFishtankQAndroidEmulatorWindowAgent =
        &sFishtankQAndroidEmulatorWindowAgent;

const QAndroidEmulatorWindowAgent* const getFishtankEmulatorWindowAgent() {
    return gFishtankQAndroidEmulatorWindowAgent;
}
