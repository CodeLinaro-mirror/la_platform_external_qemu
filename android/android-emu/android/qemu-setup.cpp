...
#include "android/hw-xrlights.h"                          // for android...
...
#include "android/xr/XrService.h"                         // for initial...
...
bool android_emulation_setup(const AndroidConsoleAgents* agents, bool isQemu2) {
    ...
    if (getConsoleAgents()->settings->android_qemu_mode()) {
        ...
        /* Register the 'xr_service' with qemud, this originally
           happened inside of the physical model, but it has since
           been decoupled and this was identified as the least
           impactful place to register the service instead */
        if (flavor == AVD_XR) {
            xr_service::initializeQemudXrService();
        }
