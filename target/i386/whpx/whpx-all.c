...
static struct AccelCPUState* get_whpx_vcpu(CPUState* cpu) {
    return cpu->accel;
}
...
static void whpx_set_xcrs_and_xss(CPUState* cpu) {
    ...
    hr = whp_dispatch.WHvSetVirtualProcessorRegisters(whpx->partition, cpu->cpu_index, &reg_name, 1,
                                                      &reg_val);
    if (FAILED(hr)) {
...
static void whpx_set_registers(CPUState* cpu) {
    struct whpx_state* whpx = &whpx_global;
    struct AccelCPUState* vcpu = get_whpx_vcpu(cpu);
    CPUX86State* env = cpu_env(cpu);
    ...
    vcxt.values[idx].FpControlStatus.FpStatus = (env->fpus & ~0x3800) | (env->fpstt & 0x7) << 11;
...
static void whpx_get_xcrs_and_xss(CPUState* cpu) {
    ...
    /* Only xcr0 is supported by the hypervisor currently */
    reg_name = WHvX64RegisterXCr0;
    hr = whp_dispatch.WHvGetVirtualProcessorRegisters(whpx->partition, cpu->cpu_index, &reg_name, 1,
                                                      &reg_val);
    if (FAILED(hr)) {
...
static HRESULT CALLBACK whpx_emu_ioport_callback(void* ctx, WHV_EMULATOR_IO_ACCESS_INFO* IoAccess) {
    MemTxAttrs attrs = {0};
    address_space_rw(&address_space_io, IoAccess->Port, attrs, (uint8_t*)&IoAccess->Data,
...
static HRESULT CALLBACK whpx_emu_getreg_callback(void* ctx, const WHV_REGISTER_NAME* RegisterNames,
    ...
    if (FAILED(hr)) {
        error_report(
                "WHPX: Failed to get virtual processor registers,"
                " hr=%08lx",
                hr);
    }
...
static HRESULT CALLBACK whpx_emu_setreg_callback(void* ctx, const WHV_REGISTER_NAME* RegisterNames,
                                                 UINT32 RegisterCount,
    ...
    if (FAILED(hr)) {
        error_report(
                "WHPX: Failed to set virtual processor registers,"
                " hr=%08lx",
                hr);
    }
...
static HRESULT CALLBACK whpx_emu_translate_callback(
        void* ctx, WHV_GUEST_VIRTUAL_ADDRESS Gva, WHV_TRANSLATE_GVA_FLAGS TranslateFlags,
        WHV_TRANSLATE_GVA_RESULT_CODE* TranslationResult, WHV_GUEST_PHYSICAL_ADDRESS* Gpa) {
    HRESULT hr;
    struct whpx_state* whpx = &whpx_global;
    CPUState* cpu = (CPUState*)ctx;
    WHV_TRANSLATE_GVA_RESULT res;

    hr = whp_dispatch.WHvTranslateGva(whpx->partition, cpu->cpu_index, Gva, TranslateFlags, &res,
                                      Gpa);
    if (FAILED(hr)) {
...
static int whpx_handle_mmio(CPUState* cpu, WHV_MEMORY_ACCESS_CONTEXT* ctx) {
    HRESULT hr;
    struct AccelCPUState* vcpu = get_whpx_vcpu(cpu);
    struct AccelCPUState vcpu_copy = *vcpu;
    WHV_MEMORY_ACCESS_CONTEXT ctx_copy = *ctx;
    WHV_EMULATOR_STATUS emu_status;

    hr = whp_dispatch.WHvEmulatorTryMmioEmulation(vcpu->emulator, cpu, &vcpu->exit_ctx.VpContext,
                                                  ctx, &emu_status);
    if (FAILED(hr)) {
    ...
    if (!emu_status.EmulationSuccessful) {
        ...
        error_report(
                "WHPX: Failed to emulate MMIO access with"
                " EmulatorReturnStatus: %u (%s%s%s%s%s%s%s%s%s%s)",
                emu_status.AsUINT32, (emu_status.EmulationSuccessful ? "EmulationSuccessful " : ""),
        ...
        error_report(
                "whpx_vcpu { emulator=%p, window_registered=%s, "
                "interruptable=%s, tpr=%llx, apic_base=%llx, "
                "interruption_pending=%s }",
                vcpu_copy.emulator, (vcpu_copy.window_registered ? "true" : "false"),
                (vcpu_copy.interruptable ? "true" : "false"), vcpu_copy.tpr, vcpu_copy.apic_base,
                (vcpu_copy.interruption_pending ? "true" : "false"));

        size = MIN(ctx_copy.InstructionByteCount, sizeof(ctx_copy.InstructionBytes));
        for (i = 0; i < size; ++i) {
        ...
        error_report(
                ...
                ctx_copy.InstructionByteCount, instruction,
                (ctx_copy.InstructionByteCount > sizeof(ctx_copy.InstructionBytes) ? "..." : ""),
                ctx_copy.AccessInfo.AccessType, ctx_copy.AccessInfo.GpaUnmapped,
                ctx_copy.AccessInfo.GvaValid, ctx_copy.AccessInfo.AsUINT32, ctx_copy.Gpa,
                ctx_copy.Gva);
...
static int whpx_handle_portio(CPUState* cpu, WHV_X64_IO_PORT_ACCESS_CONTEXT* ctx) {
    ...
    hr = whp_dispatch.WHvEmulatorTryIoEmulation(vcpu->emulator, cpu, &vcpu->exit_ctx.VpContext, ctx,
                                                &emu_status);
    if (FAILED(hr)) {
    ...
    if (!emu_status.EmulationSuccessful) {
        error_report(
                "WHPX: Failed to emulate PortIO access with"
...
static void whpx_vcpu_pre_run(CPUState* cpu) {
    ...
    if (!vcpu->interruption_pending && vcpu->interruptable && (env->eflags & IF_MASK)) {
    ...
    if (reg_count) {
        ...
        if (FAILED(hr)) {
            error_report(
                    "WHPX: Failed to set interrupt state registers,"
                    " hr=%08lx",
                    hr);
        }
    }
...
static int whpx_vcpu_run(CPUState* cpu) {
    ...
    do {
        ...
        hr = whp_dispatch.WHvRunVirtualProcessor(whpx->partition, cpu->cpu_index, &vcpu->exit_ctx,
        ...
        case WHvRunVpExitReasonX64MsrAccess: {
            ...
            reg_values[0].Reg64 =
                    vcpu->exit_ctx.VpContext.Rip + vcpu->exit_ctx.VpContext.InstructionLength;
            ...
            if (FAILED(hr)) {
                error_report(
                        "WHPX: Failed to set MsrAccess state "
                        " registers, hr=%08lx",
                        hr);
            }
            ret = 0;
        ...
        case WHvRunVpExitReasonX64Cpuid: {
            ...
            rip = vcpu->exit_ctx.VpContext.Rip + vcpu->exit_ctx.VpContext.InstructionLength;
            switch (vcpu->exit_ctx.CpuidAccess.Rax) {
            case 1:
                rax = vcpu->exit_ctx.CpuidAccess.DefaultResultRax;
                /* Advertise that we are running on a hypervisor */
                rcx = vcpu->exit_ctx.CpuidAccess.DefaultResultRcx | CPUID_EXT_HYPERVISOR;
            ...
            case 7:
                ...
                rbx = vcpu->exit_ctx.CpuidAccess.DefaultResultRbx & ~CPUID_7_0_EBX_SMAP;
            ...
            case 0x80000001:
                ...
                rcx = vcpu->exit_ctx.CpuidAccess.DefaultResultRcx & ~CPUID_EXT3_OSVW;
            ...
            if (FAILED(hr)) {
                error_report(
                        "WHPX: Failed to set CpuidAccess state registers,"
                        " hr=%08lx",
                        hr);
            }
            ret = 0;
...
int whpx_init_vcpu(CPUState* cpu) {
    ...
    vcpu = g_malloc0(sizeof(AccelCPUState));
    ...
    if (FAILED(hr)) {
        error_report(
                "WHPX: Failed to setup instruction completion support,"
                " hr=%08lx",
                hr);
        g_free(vcpu);
        return -EINVAL;
    ...
    if (FAILED(hr)) {
        error_report(
                "WHPX: Failed to create a virtual processor,"
                " hr=%08lx",
                hr);
...
static void whpx_user_backed_ram_map(uint64_t gpa, void* hva, uint64_t size, int
flags) { struct whpx_state *whpx = &whpx_global; HRESULT hr;

    hr = whpx_set_ram(hva, gpa, size, user_backed_flags_to_whpx(flags), 1 );
...
static void whpx_user_backed_ram_unmap(uint64_t gpa, uint64_t size) {
    ...
    hr = whpx_set_ram(0, gpa, size, 0 , 0 );
...
static void whpx_region_add(MemoryListener* listener, MemoryRegionSection* section) {
    memory_region_ref(section->mr);
    whpx_process_section(section, 1);
}

static void whpx_region_del(MemoryListener* listener, MemoryRegionSection* section) {
...
static void whpx_log_sync(MemoryListener* listener, MemoryRegionSection* section) {
    MemoryRegion* mr = section->mr;
    ...
    if (pRtlGetVersion) {
        ...
        if (ntStatus == 0) {
            printf("WHPX on Windows %d.%d.%d detected.\n", ovi.dwMajorVersion, ovi.dwMinorVersion,
                   ovi.dwBuildNumber);
            if (ovi.dwMajorVersion >= 10 && ovi.dwBuildNumber >= 22000) return;
        }
    }
...
static HRESULT whpx_set_exception_exit_bitmap(UINT64 exceptions) {
    ...
    hr = whp_dispatch.WHvSetPartitionProperty(whpx->partition,
                                              WHvPartitionPropertyCodeExceptionExitBitmap, &prop,
...
static void whpx_set_kernel_irqchip(Object* obj, Visitor* v, const char* name, void* opaque,
                                    Error** errp) {
    struct whpx_state* whpx = &whpx_global;
    OnOffSplit mode;
    ...
    case ON_OFF_SPLIT_SPLIT:
        ...
        error_append_hint(errp, "Try without kernel-irqchip or with kernel-irqchip=on|off");
        break;
...
static int whpx_accel_init(MachineState* ms) {
    ...
    hr = whp_dispatch.WHvGetCapability(WHvCapabilityCodeHypervisorPresent, &whpx_cap,
    ...
    hr = whp_dispatch.WHvGetCapability(WHvCapabilityCodeFeatures, &features, sizeof(features),
                                       NULL);
    if (FAILED(hr)) {
    ...
    hr = whp_dispatch.WHvGetPartitionProperty(
            whpx->partition, WHvPartitionPropertyCodeProcessorXsaveFeatures, &whpx_xsave_cap,
            sizeof(whpx_xsave_cap), &whpx_cap_size);
    ...
    if (FAILED(hr)) {
        error_report(
                "WHPX: Failed to set partition core count to %d,"
                " hr=%08lx",
                ms->smp.cores, hr);
        ret = -EINVAL;
        goto error;
    ...
    if (whpx->kernel_irqchip_required &&
        ...
        error_report(
                "WHPX: kernel irqchip requested, but unavailable. "
    ...
    if (FAILED(hr)) {
        error_report(
                "WHPX: Failed to enable partition extended X64MsrExit and"
                " X64CpuidExit hr=%08lx",
                hr);
        ret = -EINVAL;
        goto error;
    }
    ...
    hr = whp_dispatch.WHvSetPartitionProperty(whpx->partition,
                                              WHvPartitionPropertyCodeCpuidExitList, cpuidExitList,
                                              RTL_NUMBER_OF(cpuidExitList) * sizeof(UINT32));

    if (FAILED(hr)) {
        error_report(
                "WHPX: Failed to enable partition extended CpuidExitList"
                " hr=%08lx",
                hr);
        ret = -EINVAL;
        goto error;
    ...
    if (FAILED(hr)) {
        error_report(
                "WHPX: Failed to enable partition extended X64MsrExit and CpuidExit's"
                " hr=%08lx",
                hr);
        ret = -EINVAL;
        goto error;
...
static void whpx_accel_class_init(ObjectClass* oc, void* data) {
    ...
    object_class_property_add(oc, "kernel-irqchip", "on|off|split", NULL, whpx_set_kernel_irqchip,
                              NULL, NULL);
    object_class_property_set_description(oc, "kernel-irqchip", "Configure WHPX in-kernel irqchip");
}
...
bool init_whp_dispatch() {
    ...
    if (whp_dispatch_initialized) return true;

#define WHP_LOAD_FIELD(return_type, function_name, signature)                                  \
    whp_dispatch.function_name = (function_name##_t)GetProcAddress(hLib, #function_name);      \
    if (!whp_dispatch.function_name) {                                                         \
        error_report("Could not load function %s from library %s.", #function_name, lib_name); \
        goto error;                                                                            \
    }
...
error:
    ...
    if (hWinHvEmulation) FreeLibrary(hWinHvEmulation);
    return false;
}
