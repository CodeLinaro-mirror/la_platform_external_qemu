...
typedef struct MAX34451State {
    ...
    uint8_t fault_log[FAULT_LOG_SIZE];
    uint32_t time_count;
...
/** CRC16 table for polynomial 0x8005 (x^16 + x^15 + x^2 + 1) */
...
static uint16_t max34451_crc16(uint16_t crc, const uint8_t *data, size_t len)
{
...
static uint16_t max34451_crc16_flash(MAX34451State *s)
{
    PMBusDevice *pmdev = PMBUS_DEVICE(s);

    s->crc = 0;
    for (size_t i = 0; i < pmdev->num_pages; i++) {
        const uint8_t *data = (const uint8_t *)&pmdev->pages[i];
        s->crc = max34451_crc16(s->crc, data, sizeof(pmdev->pages[0]));
    }
...
static uint8_t max34451_read_byte(PMBusDevice *pmdev)
{
    ...
    case MAX34451_MFR_NV_FAULT_LOG:
        pmbus_send(pmdev, s->fault_log, FAULT_LOG_SIZE);
        break;
    ...
    case MAX34451_MFR_CRC:
        max34451_crc16_flash(s);
        pmbus_send16(pmdev, s->crc);
        break;
                               ...
                               uint8_t len)
{
     ...
     * TODO: b/462199038 - Add support for reading the status register 255.
     */
    ...
    case MAX34451_MFR_CRC:
        /*
         * This is the 16-bit page number, which indicates the type of storage
         * we should generate the CRC16 for, however we don't model the
         * different types of storage, so just toss it.
         */
        pmbus_receive16(pmdev);
        break;
    case MAX34451_MFR_NV_FAULT_LOG:
...
static void max34451_exit_reset(Object *obj, ResetType type)
{
    ...
    memset(s->fault_log, 0xff, FAULT_LOG_SIZE);
}
...
static const VMStateDescription vmstate_max34451 = {
    ...
    .fields = (const VMStateField[]){
        ...
        VMSTATE_UINT8_ARRAY(fault_log, MAX34451State, FAULT_LOG_SIZE),
...
static void max34451_init(Object *obj)
{
    ...
    for (int i = 0; i < MAX34451_NUM_PWR_DEVICES; i++) {
        object_property_add(obj, "iout[*]", "uint16",
                            max34451_get,
                            max34451_set, NULL, &pmdev->pages[i].read_iout);
    }
    ...
    /* common pmbus device registers needed for power fault injection */
    for (int i = 0; i < MAX34451_NUM_PAGES; i++) {
        object_property_add_uint16_ptr(obj, "status_word[*]",
                                       &pmdev->pages[i].status_word,
                                       OBJ_PROP_FLAG_READWRITE);
        object_property_add_uint8_ptr(obj, "status_cml[*]",
                                       &pmdev->pages[i].status_cml,
                                       OBJ_PROP_FLAG_READWRITE);
        object_property_add_uint8_ptr(obj, "status_mfr_specific[*]",
    ...
    /* mfr_channel_config */
    for (int i = 0; i < MAX34451_NUM_PWR_DEVICES; i++) {
        object_property_add_uint16_ptr(obj, "mfr_channel_config[*]",
                                       &s->channel_config[i],
                                       OBJ_PROP_FLAG_READWRITE);
    }

    /* mfr_nv_log_config */
    object_property_add_uint16_ptr(obj, "mfr_nv_log_config",
                                       &s->nv_log_config,
                                       OBJ_PROP_FLAG_READWRITE);

    /* fault log */
    for (int i = 0; i < FAULT_LOG_SIZE; i++) {
        object_property_add_uint8_ptr(obj, "fault_log[*]",
                                       &s->fault_log[i],
                                       OBJ_PROP_FLAG_READWRITE);
    }
}
