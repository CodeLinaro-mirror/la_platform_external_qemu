...
static void adm1266_exit_reset(Object *obj, ResetType type)
{
    ...
    for (int i = 0; i < ADM1266_NUM_PAGES; i++) {
        ...
        /*
         * 22 means linear mode with exponent -10. This allows us to set
         * get voltage ~3.3V by setting read_vout to 3379.
         */
        pmdev->pages[i].vout_mode = 22;
    ...
    strncpy(s->mfr_rev, s->default_mfr_rev, sizeof(s->mfr_rev));
    strncpy(s->mfr_location, ADM1266_MFR_LOCATION_DEFAULT, 5);
    strncpy(s->mfr_date, ADM1266_MFR_DATE_DEFAULT, 5);
    strncpy(s->mfr_serial, ADM1266_MFR_SERIAL_DEFAULT, 5);
                        ...
                        Error **errp)
{
    ...
    if (strncmp(name, "vout[", 5) == 0) {
        ...
        *internal = pmbus_milliunits2linear_mode(value, mode->exp);
        ...
        mode = (PMBusVoutMode *)opaque;
        *mode = *(PMBusVoutMode *)&value;
    } else {
        *internal = value;
    }
...
static void adm1266_get_mfr_rev(Object *obj, Visitor *v, const char *name, void *opaque,
                        Error **errp)
{
    ADM1266State *s = ADM1266(obj);
    char *mfr_rev = s->default_mfr_rev;

    visit_type_str(v, name, &mfr_rev, errp);
}

static void adm1266_set_mfr_rev(Object *obj, Visitor *v, const char *name, void *opaque,
                        Error **errp)
{
    char *mfr_rev;
    ADM1266State *s = ADM1266(obj);
    ...
    visit_type_str(v, name, &mfr_rev, errp);
    memcpy(s->default_mfr_rev, mfr_rev, sizeof(s->default_mfr_rev));
    /* Alloc'd by visit_type_str. */
    free(mfr_rev);
}
...
static void adm1266_init(Object *obj)
{
    ...
    object_property_add(obj, "default-mfr-rev", "string",
                        adm1266_get_mfr_rev,
                        adm1266_set_mfr_rev, NULL, s->default_mfr_rev);

    strncpy(s->default_mfr_rev, ADM1266_MFR_REVISION_DEFAULT,
            sizeof(s->default_mfr_rev));
}
